#!/usr/bin/env python3
"""Visualize and diagnose IgLM heavy-chain embedding space on FLAb Tm dataset."""
...
def build_table(df: pd.DataFrame, seq2emb: dict[str, np.ndarray]) -> tuple[np.ndarray, pd.DataFrame]:
    ...
    for _, r in df.iterrows():
        v = seq2emb.get(str(r["heavy"]).strip().upper())
        if v is None:
            continue
        X.append(v)
        rows.append({"source_file": r["source_file"], "assay_name": r.get("assay_name", ""), "y": float(r["y"])})
    if not X:
        raise RuntimeError("No rows mapped to IgLM embeddings")
...
def main() -> None:
    ...
    cache_npz = Path("output/iglm_cache/iglm_heavy_meanpool.npz")
    cache_map = Path("output/iglm_cache/iglm_heavy_meanpool_map.json")
    out_dir = Path("output/embedding_diagnostics_iglm")
    out_dir.mkdir(parents=True, exist_ok=True)

    if not cache_npz.exists() or not cache_map.exists():
        raise FileNotFoundError("IgLM cache files not found. Run train_flab_xgboost_iglm_heavy.py first.")
    ...
    plt.title("IgLM Heavy Embeddings: PCA (color=y)")
    plt.xlabel("PC1")
    plt.ylabel("PC2")
    plt.tight_layout()
    ...
    plt.colorbar(sc, label="Tm (y)")
    plt.title("IgLM Heavy Embeddings: t-SNE (color=y)")
    plt.xlabel("t-SNE1")
    plt.ylabel("t-SNE2")
    plt.tight_layout()
    ...
    plt.title("IgLM Heavy Embeddings: t-SNE (study groups)")
    plt.xlabel("t-SNE1")
    plt.ylabel("t-SNE2")
    plt.tight_layout()
